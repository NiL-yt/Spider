...
def storage(title1, time1, txt1):
    ...
    cur = conn.cursor()
    cur.execute("insert into jrnx (title, time, article, section) values (%s, %s, %s, %s)",
                (title1, time1, txt1, '今日南信'))
    cur.close()
    conn.commit()
    conn.close()
...
link1 = 'https://news.njcit.cn'
link2 = '/1531/list' + i + '.htm'
...
for page in range(1, int(all_pages) + 1):
    link_page = 'https://news.njcit.cn/1531/list' + str(page) + '.htm'
    print(link_page)
