...
def storage(title1, time1, txt1):
    ...
    cur = conn.cursor()
    cur.execute("insert into cjrh (title, time, article, section) values (%s, %s, %s, %s)",
                (title1, time1, txt1, '产教融合'))
    cur.close()
    conn.commit()
    conn.close()
...
for page in range(1, int(all_pages) + 1):
    link_page = 'https://www.njcit.cn/cjrh/list' + str(page) + '.htm'
    print(link_page)
