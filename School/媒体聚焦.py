...
def storage(title1, time1, txt1):
    ...
    cur = conn.cursor()
    cur.execute("insert into mtjj (title, time, article, section) values (%s, %s, %s, %s)",
                (title1, time1, txt1, '媒体聚焦'))
    cur.close()
    conn.commit()
    conn.close()
...
r = requests.get((link1 + link2), headers=headers, verify=False)
r.encoding = 'utf-8'
...
for page in range(1, int(all_pages) + 1):
    link_page = 'https://www.njcit.cn/mtjj/list' + str(page) + '.htm'
    print(link_page)

    re = requests.get(link_page, headers=headers, verify=False)
    re.encoding = 'utf-8'
    ...
    for href in news_list:
        url = href.get('href')
        print(link1 + url)
        res = requests.get((link1 + url), headers=headers, verify=False)
        res.encoding = 'utf-8'
