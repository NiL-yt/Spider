...
def storage(title1, time1, txt1):
    ...
    cur = conn.cursor()
    cur.execute("insert into djzx (title, time, article, section) values (%s, %s, %s, %s)",
                (title1, time1, txt1, '党建在线'))
    cur.close()
    conn.commit()
    conn.close()
...
for page in range(1, int(all_pages) + 1):
    link_page = 'https://www.njcit.cn/djzx/list' + str(page) + '.htm'
    print(link_page)
